...
router.get('/folder/:subFolder', function * () {
  ...
  console.log(this.params.subFolder, this.request.path, this.request.url)
  let urlPrefix = `${config.lenovobox.host}/v2/metadata/databox/${this.params.subFolder.split('-').join('/')}`
    ...
    return body.content.map(function (folder) {
      if (folder.is_dir) {
        return {
          ...
          itemsUrl: `${config.host}/folder/${folder.path.substr(1).split('/').join('-')}`
        }
      } else {
        return {
          title: path.basename(folder.path),
